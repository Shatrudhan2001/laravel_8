...
<head>
    ...
    <!--Start Style -->
    <link rel="stylesheet" type="text/css" href="{{ url('admin/assets/css/fonts.css') }}">
    <link rel="stylesheet" type="text/css" href="{{ url('admin/assets/css/bootstrap.min.css') }}">
    <link rel="stylesheet" type="text/css" href="{{ url('admin/assets/css/auth.css') }}">
    <!-- Favicon Link -->
    <link rel="shortcut icon" type="image/png" href="{{ url('web/images/favicon.png') }}">
     <style>
    .error{
        color: red !important;    
    }
            ...
            <div class="row align-items-center">
                ...
                <div class="col-xl-12">
                    <div class="ad-auth-content">
                        ...
                        <form action="{{ route('register') }}" method="post" id="formData">
                            @csrf
                            <center>
                                <a href="{{ url('/') }}" class="ad-auth-logo">
                                    <img src="{{ url('web/images/bsrs.png') }}" alt="" />
                                </a>
                            </center>
                            @if (Session::has('error'))
                                <div class="text-danger text-center mt-3">{{ Session::get('error') }}</div>
                            @endif
                            @if (Session::has('success'))
                                <div class="text-success text-center mt-3">{{ Session::get('success') }}</div>
                            @endif
                            <div class="ad-auth-form">
                                <div class="ad-auth-feilds mb-30">
                                    <input type="text" name="name" id="name" placeholder="Username"
                                        class="ad-input @error('name') is-invalid @enderror" required />
                                    @if ($errors->has('name'))
                                ...
                                <div class="ad-auth-feilds mb-30">
                                    <input type="text" name="email" id="email" placeholder="Email Address"
                                        class="ad-input  @error('email') is-invalid @enderror" required />
                                    @if ($errors->has('email'))
                                ...
                                <div class="ad-auth-feilds">
                                    <input type="password" placeholder="Password" name="password"
                                        class="ad-input  @error('password') is-invalid @enderror" />
                            ...
                            <div class="ad-auth-btn">
                                <button type="submit" class="ad-btn ad-login-member">Register</button>
                            </div>
