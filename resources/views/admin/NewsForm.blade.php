                             ...
                             @error('image')
                                ...
                                @if (session('status'))
                                    <div class="alert alert-success mb-2 col-lg-6 offset-3 text-center">
                                        {{ session('status') }}</div>
                                @endif
                            <form class="separate-form" method="post" action="{{ url('AddNews') }}" enctype="multipart/form-data">
                                @csrf
                                    ...
                                    <div class="row">
                                        <div class="col-xl-6 col-lg-6 col-md-12 col-sm-12 col-12">
                                            <div class="form-group">
                                                ...
                                                <input name="title" class="form-control" type="text" required placeholder="Enter title name " id="member-name" value="@if(!empty($data)){{ $data->title }} @endif">
                                            </div>
                                        </div>
                                       <div class="col-xl-6 col-lg-6 col-md-12 col-sm-12 col-12">
                                            <div class="form-group">
                                                ...
                                                <input type="file" name="image" class="form-control"  
                                                    id="member-name">
                                                @if(!empty($data))
                                                  <img src="{{ url('uploads/news/' . $data->image) }}" alt=""  width="100" class="mt-2"> @endif"
                                            </div>
                                        </div>
                                    </div>
                                    <div class="form-group">
                                        <label for="additional-msg" class="col-form-label">Descriptions</label>
                                        <textarea name="description" class="form-control description" id="additional-msg" required>@if(!empty($data)){{ $data->description }} @endif"</textarea>
                                    </div>
